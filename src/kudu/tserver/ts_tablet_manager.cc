...
#define TOMBSTONE_NOT_OK(expr, peer, msg) \
  do { \
    Status _s = (expr); /* NOLINT(performance-unnecessary-copy-initialization) */ \
...
class TabletCopyRunnable : public Runnable {
 public:
  ...
  // Cancel automatic invocation of the callback by the destructor.
  virtual void Cancel() {
    cb_invoked_ = true;
  }
...
void TSTabletManager::StartTabletCopy(
  ...
  // invoke the callback ourselves, so cancel the automatic callback mechanism.
  runnable->Cancel();
...
void TSTabletManager::Shutdown() {
  ...
  // TODO(mpercy): Cancel all outstanding tablet copy tasks.
