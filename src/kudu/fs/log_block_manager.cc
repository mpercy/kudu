...
Status LogBlockContainer::ReadContainerRecords(deque<BlockRecordPB>* records) const {
  ...
  if (PREDICT_TRUE(read_status.IsEndOfFile() || read_status.IsIncomplete())) {
    ...
    if (read_status.IsEndOfFile()) {
      return Status::OK();
    }
    // Below: "Incomplete" status. We found a partial trailing record in a
    // version of the pb container file format that can handle truncation.
    ...
    LOG(WARNING) << "Log block manager: Found partial trailing metadata record in container "
                  << ToString() << ": "
                  << "Truncating metadata file to last good offset.";
    // Truncate the metadata file so that we can continue appending to it..
    ...
    // Refresh metadata so that the writer will start appending from the new
    // position.
