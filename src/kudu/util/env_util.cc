...
DEFINE_int64(disk_reserved_bytes, 0,
             "Number of bytes to reserve on each filesystem for non-Kudu usage");
TAG_FLAG(disk_reserved_bytes, runtime);
TAG_FLAG(disk_reserved_bytes, evolving);

DEFINE_int64(disk_reserved_inodes, 0,
             "Number of inodes to reserve on each filesystem for non-Kudu usage");
TAG_FLAG(disk_reserved_inodes, runtime);
TAG_FLAG(disk_reserved_inodes, evolving);
...
DEFINE_int64(disk_reserved_inodes_free_for_testing, -1,
             "For testing only! Set to number of inodes free on each filesystem. "
             "Set to -1 to disable this test-specific override");
TAG_FLAG(disk_reserved_inodes_free_for_testing, runtime);
TAG_FLAG(disk_reserved_inodes_free_for_testing, unsafe);
...
Status AssertSufficientDiskSpace(Env *env, const std::string& path, int64_t bytes) {
  ...
  if (PREDICT_FALSE(FLAGS_disk_reserved_inodes_free_for_testing > -1)) {
    inodes_free = FLAGS_disk_reserved_inodes_free_for_testing;
  }

  if (bytes_free - bytes < FLAGS_disk_reserved_bytes) {
    return Status::ServiceUnavailable(Substitute("Insufficient disk space to allocate $0 bytes "
                                                 "under path $1 "
                                                 "($2 bytes free vs $3 bytes reserved)",
                                                 bytes, path, bytes_free,
                                                 FLAGS_disk_reserved_bytes));
  }
  if (inodes_free <= FLAGS_disk_reserved_inodes) {
    return Status::ServiceUnavailable(Substitute("Insufficient inodes under path $0 "
                                                 "($1 inodes free vs $2 inodes reserved)",
                                                 path, inodes_free, FLAGS_disk_reserved_inodes));
  }
  return Status::OK();
}
