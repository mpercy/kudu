...
TEST_F(TestEnvUtil, TestDiskSpaceCheck) {
  ...
  FLAGS_disk_reserved_bytes = frag_size * 200;
  s = env_util::AssertSufficientDiskSpace(env, path, 0);
  ASSERT_TRUE(s.IsServiceUnavailable());
  ASSERT_STR_CONTAINS(s.ToString(), "Insufficient disk space");
  FLAGS_disk_reserved_bytes_free_for_testing = -1;

  FLAGS_disk_reserved_bytes = 0;
  FLAGS_disk_reserved_inodes = 200;
  FLAGS_disk_reserved_inodes_free_for_testing = 0;
  s = env_util::AssertSufficientDiskSpace(env, path, 0);
  ASSERT_TRUE(s.IsServiceUnavailable());
