...
Status RoutingTable::ConstructForest(
  ...
  for (const auto& edge : proxy_topology.proxy_edges()) {
    if (ContainsKey(dest_to_proxy_from, edge.peer_uuid())) {
      return Status::InvalidArgument("invalid proxy topology: duplicate destination uuid",
                                     edge.peer_uuid());
    }
  ...
  for (const RaftPeerPB& peer : raft_config.peers()) {
    ...
    tmp_index.emplace(peer.permanent_uuid(), hop.get());
    auto result = tmp_forest.emplace(peer.permanent_uuid(), std::move(hop));
    if (!result.second) {
      return Status::InvalidArgument("invalid raft config: duplicate peer uuid",
                                     peer.permanent_uuid());
    }
  }
  ...
  for (const RaftPeerPB& peer : raft_config.peers()) {
    ...
    if (!proxy_from_uuid || proxy_from_uuid->empty()) {
      continue;
    }
    ...
    DCHECK(result.second) << "invalid config: duplicate uuid: " << node_uuid;
  }
...
void RoutingTable::ToStringHelperRec(Node* cur, int level, std::string* out) const {
  ...
  *out += strings::Substitute("$0 ($1)\n",
            ...
            pb_util::SecureShortDebugString(cur->peer_pb.last_known_addr()));
