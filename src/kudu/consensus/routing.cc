...
Status RoutingTable::Init(const RaftConfigPB& raft_config,
  ...
  RETURN_NOT_OK(ConstructForest(raft_config, proxy_topology, &index, &trees));
...
Status DurableRoutingTable::UpdateProxyGraph(ProxyTopologyPB proxy_topology) {
  ...
  if (leader_uuid_) {
    RETURN_NOT_OK(routing_table.Init(raft_config_, proxy_topology, *leader_uuid_));
  }
...
Status DurableRoutingTable::UpdateRaftConfig(RaftConfigPB raft_config) {
  ...
  if (leader_in_config) {
    RETURN_NOT_OK(routing_table.Init(raft_config, proxy_topology_, *leader_uuid_));
