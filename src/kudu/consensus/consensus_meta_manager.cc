...
Status ConsensusMetadataManager::CreateDRT(std::string tablet_id,
...
Status ConsensusMetadataManager::LoadDRT(std::string tablet_id,
                                         RaftConfigPB raft_config,
  ...
  RETURN_NOT_OK_PREPEND(DurableRoutingTable::Load(fs_manager_,
                                                  ...
                                                  std::move(raft_config),
                                                  &drt),
                        Substitute("Unable to load durable routing table for tablet $0", tablet_id));
...
Status ConsensusMetadataManager::LoadOrCreateDRT(std::string tablet_id,
                        RaftConfigPB raft_config,
                        ProxyGraphPB proxy_graph,
                        std::shared_ptr<DurableRoutingTable>* drt_out) {
  Status s = LoadDRT(tablet_id, std::move(raft_config), drt_out);
  if (s.IsNotFound()) {
    return CreateDRT(tablet_id, raft_config, proxy_graph, drt_out);
  }
  return s;
}
