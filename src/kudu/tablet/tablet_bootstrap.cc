...
Status TabletBootstrap::Bootstrap(shared_ptr<Tablet>* rebuilt_tablet,
  ...
  CHECK((tablet_ && log_) || !bootstrap_status.ok()) << "Tablet and Log not initialized";
  if (tablet_ && log_) {
    meta_->SetPreFlushCallback(
        Bind(&FlushInflightsToLogCallback::WaitForInflightsAndFlushLog,
             make_scoped_refptr(new FlushInflightsToLogCallback(tablet_.get(), log_))));
  }

  // This will cause any pending TabletMetadata flush to be executed, if any.
