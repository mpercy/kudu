  ...
  *is_history_truncated = false;
  ...
  for (const Mutation *redo_mut = src_row.redo_head;
    ...
    if (redo_decoder.is_update()) {
      ...
      if (history_gc_opts.do_history_gc == HistoryGCOpts::GC_HISTORY &&
          redo_mut->timestamp().CompareTo(history_gc_opts.ancient_history_mark) < 0) {
  ...
  // Also GC old undos if GC is enabled.
  // TODO: Move this to the top, before converting redos to undos, so we don't
  // have to traverse more mutations than required.
  if (history_gc_opts.do_history_gc == HistoryGCOpts::GC_HISTORY) {
    Mutation *prev_undo = nullptr;
    for (const Mutation *undo_mut = undo_head;
         undo_mut != nullptr;
         undo_mut = undo_mut->next()) {
      if (undo_mut->timestamp().CompareTo(history_gc_opts.ancient_history_mark) < 0) {
  ...
  while (input->HasMoreBlocks()) {
    ...
    for (const CompactionInputRow &input_row : rows) {
      ...
      DVLOG(2) << "Output Row: " << dst_row.schema()->DebugRow(dst_row) <<
        ...
        " Was history truncated? " << is_history_truncated;
