...
Status DiskRowSet::MajorCompactDeltaStores(const HistoryGCOpts& history_gc_opts) {
  ...
  return MajorCompactDeltaStoresWithColumnIds(col_ids, history_gc_opts);
}
...
Status DiskRowSet::MajorCompactDeltaStoresWithColumnIds(const vector<ColumnId>& col_ids,
  ...
  RETURN_NOT_OK(NewMajorDeltaCompaction(col_ids, history_gc_opts, &compaction));
