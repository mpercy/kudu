...
To build the documentation or the site, the pre-requisites for building Kudu
itself must be installed, as well as the a few more system packages:

[source,bash]
----
$ sudo apt-get install ruby ruby-devel xsltproc # for Ubuntu
----

(Or the equivalent for other platforms.)

Then, to build the docs in a form that is convenient for testing changes to the
.adoc source files, but not in a way that integrates with the look and feel of
Kudu web site, run the following command:
...
This will invoke `docs/support/scripts/make_docs.sh`, and will place the
generated HTML files in the _build/docs_ subdirectory.

=== Updating the documentation on the Kudu web site

To update the documentation that is integrated into the Kudu web site,
including Javadoc documentation, first install the docs pre-requisites as
described above, then run the following command:
...
You can then proceed to commit the changes on your local `gh-pages` branch and
send a Gerrit code review for that branch.

You can submit a Gerrit code review for the `gh-pages` branch using
command-line syntax like the following:

[source,bash]
----
git push --no-thin gerrit HEAD:refs/for/gh-pages
----
