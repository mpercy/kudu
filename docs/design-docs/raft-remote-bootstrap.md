...
# Kudu config change: Master & remote bootstrap integration
...
2. mkdir quarantine directory (QDIR).
...
1. Master sends an AddServer() RPC to the leader of the tablet to add a new
   replica as a PRE_VOTER (the initial implementation may simply add the peer
   as a VOTER).
   a. This AddServer() RPC will specify the prior committed raft config for the
      tablet to ensure that the request is idempotent.
   b. The master will periodically send such RPCs until it sees that it has
...
to "config change" operations (AddServer / RemoveServer / ChangeRole). We will
add committed_config_id as an optional parameter that identifies the latest
committed consensus config. If the opid doesn't match the currently committed
config on the peer (checked under the RaftConsensus ReplicaState lock), the RPC
returns an error indicating that the caller is out of date. If the OpId
matches, but there is already a pending config change, the request is also
...
VOTER, the opportunity for potential problems with VOTERs is reduced a lot,
