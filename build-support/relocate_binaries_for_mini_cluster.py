...
def check_for_command(command):
    ...
    try:
        ...
        print("Unable to find %s command" % (command,), file=sys.stderr)
        raise err
...
def get_dep_library_paths_linux(binary_path):
    ...
    libs = []
    for line in lines:
        # ldd output lines generally look like the following:
        # libpthread.so.0 => /lib/x86_64-linux-gnu/libpthread.so.0 (0x00007f1dcd8fc000)
        components = re.split('\s+', line.strip())
        if len(components) < 3:
            # Probably the dynamic linker, e.g. /lib64/ld-linux-x86-64.so.2
            continue
        lib = components[2]
        if not os.path.exists(lib):
            # Probably a virtual dynamic shared object, e.g. linux-vdso.so.1
            continue
        libs.append(lib)
    return libs
...
def relocate_deps_linux(target, config):
    ...
    for lib_src in libs:
        ...
        copy_library(lib_src, lib_dst)

    # Make sure we have the chrpath command available.
    ...
    try:
        ...
        print("Failed to chrpath for target %s" % target_dst, file=sys.stderr)
        raise err
...
def main():
    ...
    if not os.path.exists(build_root):
        raise NotADirectoryError("Build directory %s does not exist" % (build_root, ))

    artifact_name = get_artifact_name()
    artifact_root = os.path.join(build_root, artifact_name)

    print("Including targets and their dependencies in archive...")
