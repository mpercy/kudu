...
# release mode with dynamic linking so that the kudu-tserver and kudu-master
# executables can use the same bit for their dependencies.
...
$SOURCE_ROOT/build-support/relocate_binaries_for_mini_cluster.py $BUILD_ROOT
...
This archive contains Kudu server binaries for use in a "mini cluster" by unit
tests only.
